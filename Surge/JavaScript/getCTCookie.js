function getUrl() {
    if ($request.url.indexOf("https://e.dlife.cn/user/loginMiddle") !== -1) {
        ...
        $persistentStore.write(login_url, `@LordVader.china_telecom_loginUrl`);
        $notification.post("写入天翼账号登录URL成功 🎉", "", login_url, {
...
function getCookie() {
    ...
    $persistentStore.write(headerCookie, `@LordVader.china_telecom_cookie`);
    $notification.post("写入天翼账号Cookie成功 🎉", "", headerCookie, {
        "media-url": "https://raw.githubusercontent.com/anker1209/icon/main/zgdx.png",
        "auto-dismiss": 10
    })
}
